...
import { toFriendly } from "@/utils/errorHandler";
...
export function BookCall() {
  ...
  const submit = async () => {
    ...
    try {
      const { submitCallback } = await import("@/lib/leads");
      const res = await submitCallback({
        name: data.name, phone: data.phone, service: data.service,
        preferred_date: data.date, preferred_time: data.time,
        message: null,
      });
      if (!res.ok) {
        const f = toFriendly(res.error);
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-soft md:p-10">
          ...
          <motion.div key="form" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }}>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <Field label="Service" icon={<Calendar size={16} />}>
                <select value={data.service} onChange={set("service")} className="input">
                  {services.map((s) => <option key={s.slug}>{s.title}</option>)}
                </select>
              </Field>
              <Field label="Date" icon={<Calendar size={16} />}>
                <input type="date" min={today} value={data.date} onChange={set("date")} className="input" />
              </Field>
              <Field label="Time Slot" icon={<Clock size={16} />}>
                <select value={data.time} onChange={set("time")} className="input">
                  {slots.map((s) => <option key={s}>{s}</option>)}
                </select>
              </Field>
              <Field label="Your Name" icon={<User size={16} />}>
                <input type="text" maxLength={60} value={data.name} onChange={set("name")} placeholder="e.g. Rohan Sharma" className="input" />
              </Field>
              <Field label="Mobile Number" icon={<Phone size={16} />}>
                <input type="tel" maxLength={15} value={data.phone} onChange={set("phone")} placeholder="+91 ..." className="input" />
              </Field>
            </div>
      ...
      <style>{`
        .input { width:100%; border-radius:0.75rem; border:1px solid var(--color-border); background:var(--color-background); padding:0.65rem 0.85rem; font-size:0.9rem; outline:none; transition:border-color .2s, box-shadow .2s; }
