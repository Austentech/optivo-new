...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact Optivo Solutions" },
      { name: "description", content: "Get in touch with Optivo Solutions. Call, WhatsApp, email or visit our office." },
      { property: "og:title", content: "Contact Optivo Solutions" },
      { property: "og:description", content: "Let's talk growth — call, WhatsApp or visit us." },
...
function ContactForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const { submitEnquiry } = await import("@/lib/leads");
      const { toast } = await import("sonner");
      const { toFriendly } = await import("@/utils/errorHandler");
      ...
      toast.success("Message sent", { description: "We'll reply within 30 minutes." });
    } catch (err) {
      const { toast } = await import("sonner");
      const { toFriendly } = await import("@/utils/errorHandler");
