...
export default function EngagementPopup() {
  ...
  const close = () => {
    setOpen(false);
    try { sessionStorage.setItem("optivo:popup", "1"); } catch {}
  };
        ...
        >
          ...
          >
            <button
              ...
              className="absolute right-3 top-3 z-10 grid h-10 w-10 place-items-center rounded-full bg-background/80 text-muted-foreground transition hover:bg-secondary hover:text-foreground"
            >
              <X size={18} />
            </button>
            ...
            <div className="relative">
              ...
              <div className="mt-6 flex flex-col gap-2 sm:flex-row">
                <a
                  href={waLink(`Hi Optivo, ${m.title.toLowerCase()} — please share details.`)}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-[#25D366] px-5 py-3 text-sm font-semibold text-white shadow-glow"
                  onClick={close}
                >
                ...
                <a
                  href="/contact"
                  onClick={close}
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-gradient-brand px-5 py-3 text-sm font-semibold text-white"
                >
